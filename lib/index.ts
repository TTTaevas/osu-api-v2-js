...
export class APIError extends Error {
  ...
  constructor(
    ...
    super();
    if (this.parameters?.client_secret) {
      this.parameters.client_secret = "<REDACTED>";
    }
    if (this.parameters?.refresh_token) {
      this.parameters.refresh_token = "<REDACTED>";
    }
  }
}
...
export class API {
  ...
  constructor(
    client_id_or_settings: API["client_id"] | Partial<API>,
    client_secret?: API["client_secret"],
    redirect_uri_or_settings?: string | Partial<API>,
    code?: string,
    settings?: Partial<API>,
  ) {
    settings ??=
      typeof redirect_uri_or_settings === "string" || !redirect_uri_or_settings
        ? typeof client_id_or_settings === "number"
          ? undefined
          : client_id_or_settings
        : redirect_uri_or_settings;

    if (settings) {
      /** Delete every property that is `undefined` so the class defaults aren't overwritten by `undefined` */
      Object.keys(settings).forEach((key) => {
        settings[key as keyof API] === undefined
          ? delete settings[key as keyof API]
          : {};
      });
      Object.assign(this, settings);
    }

    if (typeof client_id_or_settings === "number")
      this.client_id = client_id_or_settings;
    ...
    if (
      this.set_token_on_creation &&
      !this.access_token.length &&
      this.client_id > 0 &&
      this.client_secret.length
    ) {
  ...
  set route_api(route_api) {
    this._route_api = route_api;
  }
   ...
   */
  public async setNewToken(authorization?: {
    redirect_uri: string;
    code: string;
  }): Promise<boolean> {
    ...
    this.token_promise = new Promise((resolve, reject) => {
      ...
      const grant_type = authorization
        ? "authorization_code"
        : this.refresh_token
          ? "refresh_token"
          : "client_credentials";
      const client_credentials_scope = this.scopes.includes("delegate")
        ? this.scopes.join(" ")
        : ("public" as Scope);
        ...
        .then((response) => {
          response.json().then((json: any) => {
            if (!json.access_token) {
              const error_message =
                json.error_description ?? json.message ?? "No token obtained"; // Expect "Client authentication failed"
              this.log(
                true,
                "Unable to obtain a token! Here's what was received from the API:",
                json,
              );
              reject(
                new APIError(
                  ...
                  this.route_token,
                  body,
                  { status_code: response.status, json },
                ),
              );
            }
            ...
            expiration_date.setSeconds(
              expiration_date.getSeconds() + json.expires_in,
            );
   ...
   */
  private async fetch(
    is_token_related: boolean,
    method: "get" | "post" | "put" | "delete",
    endpoint: Array<string | number>,
    parameters: { [k: string]: any } = {},
    info: { number_try: number; has_new_token: boolean } = {
      number_try: 1,
      has_new_token: false,
    },
  ): Promise<Response> {
          ...
          if (response.status === 401) {
            if (this.set_token_on_401 && !info.has_new_token) {
              if (!this.is_setting_token) {
                ...
                this.log(
                  true,
                  "A new token is currently being obtained!",
                  request_id,
                );
              ...
              this.log(
                true,
                "Maybe you need to do this action as a user?",
                request_id,
              );
            }
            ...
            this.log(
              true,
              "You may lack the necessary scope for this action!",
              request_id,
            );
          } else if (response.status === 422) {
            this.log(
              true,
              "You may be unable to use those parameters together!",
              request_id,
            );
          } else if (response.status === 429) {
            this.log(
              true,
              "You're sending too many requests at once and are getting rate-limited!",
              request_id,
            );
          }
        }
    ...
    if (to_retry === true && info.number_try <= this.retry_maximum_amount) {
      this.log(
        true,
        `Will request again in ${this.retry_delay} seconds...`,
        `(retry #${info.number_try}/${this.retry_maximum_amount})`,
        request_id,
      );
  ...
  public async request(
    ...
    try {
      ...
      throw new APIError(
        `${e?.name} (${e?.message ?? e?.errno ?? e?.type})`,
        `${this.server}/${this.route_api.join("/")}`,
        method,
        endpoint,
        parameters,
        undefined,
  ...
  readonly getBeatmapDifficultyAttributesOsu =
    Beatmap.DifficultyAttributes.getOsu;

  /** {@inheritDoc Beatmap.DifficultyAttributes.getTaiko} @group Beatmap Methods */
  readonly getBeatmapDifficultyAttributesTaiko =
    Beatmap.DifficultyAttributes.getTaiko;

  /** {@inheritDoc Beatmap.DifficultyAttributes.getFruits} @group Beatmap Methods */
  readonly getBeatmapDifficultyAttributesFruits =
    Beatmap.DifficultyAttributes.getFruits;

  /** {@inheritDoc Beatmap.DifficultyAttributes.getMania} @group Beatmap Methods */
  readonly getBeatmapDifficultyAttributesMania =
    Beatmap.DifficultyAttributes.getMania;

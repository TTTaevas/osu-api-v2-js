...
export namespace Beatmap {
  ...
  export interface WithBeatmapsetChecksumMaxcombo
    extends WithBeatmapset, WithChecksum {
    /**
  ...
  export namespace Extended {
    ...
    export interface WithFailtimesOwnersMaxcombo
      extends WithFailtimesOwners, WithMaxcombo {}
  ...
  export namespace Pack {
    ...
    ): Promise<Pack.WithBeatmapset> {
      const tag = typeof pack === "string" ? pack : pack.tag;
      return await this.request("get", ["beatmaps", "packs", tag], {
        legacy_only: Number(legacy_only),
      });
    }
    ...
    export async function getMultiple(
      ...
      return await this.request("get", ["beatmaps", "packs"], {
        type,
        cursor_string,
      });
    }
  ...
  export namespace DifficultyAttributes {
    ...
    export async function getOsu(
      ...
      beatmap: Beatmap["id"] | Beatmap,
      mods?: Mod[] | string[] | number,
    ): Promise<DifficultyAttributes.Osu> {
      return (await this.getBeatmapDifficultyAttributes(
        beatmap,
        mods,
        Ruleset.osu,
      )) as DifficultyAttributes.Osu;
    }
     ...
     * @param mods Can be a bitset of mods, an array of mod acronyms, or an array of Mods (ignores mod settings) (defaults to **No Mod**)
     */
    export async function getTaiko(
      this: API,
      beatmap: Beatmap["id"] | Beatmap,
      mods?: Mod[] | string[] | number,
    ): Promise<DifficultyAttributes.Taiko> {
      return (await this.getBeatmapDifficultyAttributes(
        beatmap,
        mods,
        Ruleset.taiko,
      )) as DifficultyAttributes.Taiko;
    }
    /**
    ...
    export async function getFruits(
      ...
      beatmap: Beatmap["id"] | Beatmap,
      mods?: Mod[] | string[] | number,
    ): Promise<DifficultyAttributes.Fruits> {
      return (await this.getBeatmapDifficultyAttributes(
        beatmap,
        mods,
        Ruleset.fruits,
      )) as DifficultyAttributes.Fruits;
    }
    /**
     ...
     * @remarks Since the pp update of 2025-03-06, no property exclusive to this Ruleset exists
     */
    export async function getMania(
      this: API,
      beatmap: Beatmap["id"] | Beatmap,
      mods?: Mod[] | string[] | number,
    ): Promise<DifficultyAttributes.Mania> {
      return (await this.getBeatmapDifficultyAttributes(
        beatmap,
        mods,
        Ruleset.mania,
      )) as DifficultyAttributes.Mania;
    }
  }
  ...
  export async function getUserScore(
    ...
    beatmap: Beatmap["id"] | Beatmap,
    user: User["id"] | User,
    config?: Pick<Config, "ruleset" | "mods" | "legacy_only">,
  ): Promise<{
    ...
    const mode =
      config?.ruleset !== undefined ? Ruleset[config.ruleset] : undefined;
  ...
  export async function getUserScores(
    ...
    beatmap: Beatmap["id"] | Beatmap,
    user: User["id"] | User,
    config?: Pick<Config, "ruleset" | "legacy_only">,
  ): Promise<Score[]> {
    const ruleset =
      config?.ruleset !== undefined ? Ruleset[config.ruleset] : undefined;
    delete config?.ruleset;
  ...
  export async function getMultiple(
    this: API,
    beatmaps: Array<Beatmap["id"] | Beatmap>,
  ): Promise<Extended.WithFailtimesOwnersMaxcomboBeatmapset[]> {
    const ids = beatmaps.map((beatmap) =>
      typeof beatmap === "number" ? beatmap : beatmap.id,
    );

...
export namespace Chat {
  ...
  export namespace Channel {
    ...
    ): Promise<Channel.WithDetails> {
      const channel_id =
        typeof channel === "number" ? channel : channel.channel_id;
    ...
    export async function markAsRead(
      ...
      const channel_id =
        typeof channel === "number" ? channel : channel.channel_id;
      const message_id =
        typeof message === "number" ? message : message.message_id;
      return await this.request(
        "put",
        ["chat", "channels", channel_id, "mark-as-read", message_id],
    ...
    export async function createPrivate(
      this: API,
      user_target: IUser["id"] | IUser,
    ): Promise<Channel.WithRecentmessages> {
      const target_id =
        typeof user_target === "number" ? user_target : user_target.id;
      return await this.request("post", ["chat", "channels"], {
        type: "PM",
        target_id,
      });
    }
    ...
    export async function createAnnouncement(
      ...
      user_targets: Array<IUser["id"] | IUser>,
      message: string,
    ): Promise<Channel> {
      const target_ids = user_targets.map((user) =>
     ...
     */
    export async function leaveOne(
      this: API,
      channel: Channel["channel_id"] | Channel,
      user?: IUser["id"] | IUser,
    ): Promise<void> {
  ...
  export namespace Message {
    ...
    export async function getMultiple(
      ...
      channel: Channel["channel_id"] | Channel,
      limit: number = 20,
      since?: Message["message_id"] | Message,
      until?: Message["message_id"] | Message,
    ): Promise<Message.WithSender[]> {
      const channel_id =
        typeof channel === "number" ? channel : channel.channel_id;
    ...
    export async function sendPrivate(
      ...
      const target_id =
        typeof user_target === "number" ? user_target : user_target.id;
      return await this.request("post", ["chat", "new"], {
        target_id,
        message,
        is_action,
        uuid,
  ...
  export namespace Websocket {
    ...
    export function generate(
      ...
      server = "wss://notify.ppy.sh",
    ): WebSocket {
      return new WebSocket(server, {
        headers: headers ?? this.getChatWebsocketHeaders(),
      });
    }
  }
  ...
  export async function keepAlive(
    ...
    const history_since =
      typeof since?.user_silence === "object"
        ? since.user_silence.id
        : since?.user_silence;
    const message_since =
      typeof since?.message === "object"
        ? since.message.message_id
        : since?.message;
    const response = await this.request("post", ["chat", "ack"], {
      history_since,
      since: message_since,
    });

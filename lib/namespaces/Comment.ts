...
function removeDeletedItems<T extends Comment.Bundle>(bundle: T): T {
  ...
  bundle.deleted_commentable_meta =
    bundle.commentable_meta.length - commentable_meta.length;
...
export namespace Comment {
  ...
  export async function getOne(
    ...
    return removeDeletedItems(
      await this.request("get", ["comments", comment_id]),
    );
  }
  ...
  export async function getMultiple(
    this: API,
    from?: { type: Comment["commentable_type"]; id: number },
    ...
    return removeDeletedItems(
      await this.request("get", ["comments"], {
        after,
        commentable_type: from?.type,
        commentable_id: from?.id,
        cursor: sort?.cursor,
        parent_id,
        sort: sort?.type,
      }),

...
export namespace Forum {
  ...
  export namespace Post {
    ...
    export async function edit(
      ...
      return await this.request("put", ["forums", "posts", post_id], {
        body: new_text,
      });
    }
  }
  ...
  export namespace Topic {
    ...
    export async function getOne(
      ...
      const start =
        config?.sort !== "id_desc"
          ? typeof config?.first_post === "object"
            ? config.first_post.id
            : config?.first_post
          : undefined;
      const end =
        config?.sort === "id_desc"
      ...
      const sort =
        config?.sort === "id_asc"
          ...
          : config?.sort === "id_desc"
            ...
            : undefined;
      const forum_id =
        typeof config?.forum === "object" ? config.forum.id : config?.forum;
      return await this.request("get", ["forums", "topics"], {
        limit: config?.limit,
        cursor_string: config?.cursor_string,
        sort,
        forum_id,
      });
    }
    ...
    export async function create(
      ...
      const options =
        poll?.options !== undefined
          ? poll.options.toString().replace(/,/g, "\n")
          : undefined;

      return await this.request("post", ["forums", "topics"], {
        forum_id,
        title,
        body: text,
        with_poll,
    ...
    export async function editTitle(
      ...
      const topic_id = typeof topic === "number" ? topic : topic.id;
      return await this.request("put", ["forums", "topics", topic_id], {

...
export namespace Match {
  ...
  export async function getOne(
    ...
    const before =
      typeof query?.before === "object" ? query.before.id : query?.before;
    const after =
      typeof query?.after === "object" ? query.after.id : query?.after;
    return await this.request("get", ["matches", match_id], {
      before,
      after,
      limit: query?.limit,
    });
  ...
  export async function getMultiple(
    this: API,
    config?: Pick<Miscellaneous.Config, "limit" | "sort"> & {
      /**
    ...
    const match_id =
      typeof config?.first_match_in_array === "object"
        ? config?.first_match_in_array.id
        : config?.first_match_in_array;
    const cursor = match_id
      ? { match_id: match_id + (config?.sort === "id_asc" ? -1 : 1) }
      : undefined;

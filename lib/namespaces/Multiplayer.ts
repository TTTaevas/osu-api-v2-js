  ...
  export namespace Room {
    ...
    export namespace PlaylistItem {
      ...
      export async function getScores(
        this: API,
        item:
          | { id: PlaylistItem["id"]; room_id: Room["id"] }
          | Multiplayer.Room.PlaylistItem,
        ...
        return await this.request(
          "get",
          ["rooms", item.room_id, "playlist", item.id, "scores"],
          { limit, sort, cursor_string: config?.cursor_string },
        );
      }
    }
    ...
    export async function getOne(
      this: API,
      room: Room["id"] | Room,
    ): Promise<Room.WithHostRecentparticipants> {
      const room_id = typeof room === "number" ? room : room.id;

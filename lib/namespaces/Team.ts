...
export namespace Team {
	 ...
	 */
	export async function getOne(this: API, team: Team["id"] | Team["short_name"] | Team, ruleset?: Ruleset): Promise<Extended> {
		const mode = ruleset !== undefined ? Ruleset[ruleset] : ""
		if (typeof team === "string") {team = "@" + team} // `team` is the short_name, so use the @ prefix
		if (typeof team === "object") {team = team.id}

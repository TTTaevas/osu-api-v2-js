...
export namespace User {
  ...
  export interface WithCountryCoverGroupsTeam
    extends WithCountryCoverTeam, WithGroups {}
  ...
  export interface WithCountryCoverGroupsTeamStatisticsSupport
    extends WithCountryCoverTeam, WithGroups {
  ...
  export interface Extended
    extends WithCountryCoverGroupsTeamStatisticsSupport, WithKudosu {
    discord: string | null;
  ...
  export namespace Extended {
    ...
    export interface WithStatisticsrulesets
      extends Extended, User.WithCountryCoverGroupsTeamStatisticsrulesets {
  ...
  export namespace Kudosu {
    ...
    export async function getHistory(
      ...
      user: User["id"] | User,
      config?: Config,
    ): Promise<Kudosu.History[]> {
      const user_id = typeof user === "number" ? user : user.id;
      return await this.request("get", ["users", user_id, "kudosu"], {
        ...config,
      });
    }
  ...
  export async function getResourceOwner(
    this: API,
    ruleset?: Ruleset,
  ): Promise<User.Extended.WithStatisticsrulesets> {
  ...
  export async function getOne(
    ...
    if (typeof user === "string") {
      user = "@" + user; // `user` is the username, so use the @ prefix
    }
  ...
  export async function lookupMultiple(
    ...
    ruleset?: Ruleset,
  ): Promise<User.WithCountryCoverGroupsTeamGlobalrank[]> {
    const ids = users.map((user) =>
      typeof user === "number" ? user : user.id,
    );
    const response = await this.request("get", ["users", "lookup"], {
      ids,
      ruleset_id: ruleset,
    });
  ...
  export async function getScores(
    ...
    ruleset?: Ruleset,
    include?: { lazer?: boolean; fails?: boolean },
    config?: Config,
  ): Promise<Score.WithUserBeatmapBeatmapset[]> {
    const user_id = typeof user === "number" ? user : user.id;
    const mode = ruleset !== undefined ? Ruleset[ruleset] : undefined;
    const legacy_only =
      include?.lazer !== undefined ? Number(!include.lazer) : undefined;
    const include_fails =
      include?.fails !== undefined ? String(Number(include.fails)) : undefined;
  ...
  export async function getBeatmaps(
    ...
    const user_id = typeof user === "number" ? user : user.id;
    return await this.request("get", ["users", user_id, "beatmapsets", type], {
      ...config,
    });
  }
   ...
   */
  export async function getPassedBeatmaps(
    this: API,
    user: User["id"] | User,
    beatmapsets: Array<Beatmapset["id"] | Beatmapset>,
    config?: {
    ...
    const user_id = typeof user === "number" ? user : user.id;
    const beatmapset_ids = beatmapsets.map((beatmapset) =>
      typeof beatmapset === "number" ? beatmapset : beatmapset.id,
    );
    ...
    const response = await this.request(
      ...
      { beatmapset_ids, ruleset_id, ...config },
    );
  ...
  export async function getMostPlayed(
    ...
    user: User["id"] | User,
    config?: Config,
  ): Promise<Beatmap.Playcount[]> {
    const user_id = typeof user === "number" ? user : user.id;
    return await this.request(
      "get",
      ["users", user_id, "beatmapsets", "most_played"],
      { ...config },
    );
  }
  ...
  export async function getRecentActivity(
    ...
    user: User["id"] | User,
    config?: Config,
  ): Promise<Event.AnyRecentActivity[]> {
    const user_id = typeof user === "number" ? user : user.id;
    return await this.request("get", ["users", user_id, "recent_activity"], {
      ...config,
    });
  }
   ...
   */
  export async function getFavouriteBeatmapsetsIds(
    this: API,
  ): Promise<Beatmapset["id"][]> {

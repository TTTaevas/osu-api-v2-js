...
async function startRunningTests(
  id: number,
  secret: string,
  redirect_uri: string,
): Promise<void> {
...
const env_id =
  server === "https://osu.ppy.sh" ? process.env.ID : process.env.DEV_ID;
const env_secret =
  server === "https://osu.ppy.sh" ? process.env.SECRET : process.env.DEV_SECRET;
if (env_id === undefined) {
  throw new Error(
    "❌ The ID for this server has not been defined in the environment variables!",
  );
}
if (env_secret === undefined) {
  throw new Error(
    "❌ The SECRET for this server has not been defined in the environment variables!",
  );
}
if (process.env.REDIRECT_URI === undefined) {
  throw new Error(
    "❌ The REDIRECT_URI has not been defined in the environment variables!",
  );
}

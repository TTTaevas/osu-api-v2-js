...
const getChatChannels: Test = async (api) => {
  ...
  console.log(
    "| Further Chat tests will be made on Chat.Channel",
    test_channel.name,
    "with id",
...
const joinChatChannel: Test = async (api) => {
  ...
  if (test_channel) {
    Object.keys(test_channel).forEach((k) => {
      ...
      expect(test_channel![k as keyof Chat.Channel]).to.equal(
        channel[k as keyof Chat.Channel],
      );
    });
  } else {
    console.warn("⚠️ No test_channel to compare channel with");
  }
...
const getChatMessages: Test = async (api) => {
  ...
  messages.forEach((message) =>
    expect(message.sender_id).to.equal(message.sender.id),
  );
...
const createChatPrivateChannel: Test = async (api) => {
  ...
  pm_channel = channel;
  console.log(
    "| Further Chat tests will be made on PM Chat.Channel",
    pm_channel.name,
    "with channel_id",
    pm_channel.channel_id,
  );
  return true;
};
  ...
  if (pm_channel) {
    ...
    Object.keys(pm_channel).forEach((k) => {
      if (k !== "recent_messages") {
        console.log("| Comparing key:", k);
        expect(pm_channel![k as keyof Chat.Channel]).to.equal(
          response.channel[k as keyof Chat.Channel],
        );
      }
    });
  } else {
    console.warn("⚠️ No pm_channel to compare channel with");
  }
...
const keepChatAlive: Test = async (api) => {
  ...
  if (silences.length < 1) {
    console.log("⚠️ Note: Empty array received, no silence to check");
  }

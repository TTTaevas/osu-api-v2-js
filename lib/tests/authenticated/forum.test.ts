...
const createForumTopic: Test = async (api) => {
  const response = await api.createForumTopic(
    85,
    "osu-api-v2-js test post",
    `Please ignore this forum post
It was automatically made for the sole purpose of testing [url=https://github.com/TTTaevas/osu-api-v2-js]osu-api-v2-js[/url]`,
  ...
  console.log(
    "| Further Forum tests will be made on Forum.Topic",
    test_topic.title,
    "with id",
    test_topic.id,
  );
...
const editForumTopicTitle: Test = async (api) => {
  const topic = await api.editForumTopicTitle(
    test_topic!,
    "osu-api-v2-js test post!",
  );
...
const editForumPost: Test = async (api) => {
  const post = await api.editForumPost(
    test_post!,
    test_post!.body.raw.concat(" <3"),
  );
...
export const tests = [createForumTopic, editForumTopicTitle, editForumPost];

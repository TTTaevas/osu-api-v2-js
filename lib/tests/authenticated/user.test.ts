...
const getResourceOwner: Test = async (api) => {
  ...
  try {
    ...
    console.log(
      "| If the above error is about `cover_url` not being a string, this is safe to ignore (dev server difference)",
    );
    console.log(
      "| Otherwise, you should at least investigate what's happening",
    );
...
const getFriends: Test = async (api) => {
  ...
  relations.forEach((relation) =>
    expect(relation.target.id).to.equal(relation.target_id),
  );
  relations.forEach((relation) =>
    expect(relation.relation_type).to.equal("friend"),
  );
...
export const tests = [getResourceOwner, getFriends, getFavouriteBeatmapsetsIds];

    ...
    if (Array.isArray(obj)) {
      for (let i = 0; i < obj.length; i++) {
        ...
        if (validator.errors?.length) {
          console.error(
            obj[i],
            util.inspect(validator.errors, { colors: true, depth: 5 }),
          );
          return false;
        }
      }
...
export const runTests = async (api: API, domains: Test[][]): Promise<void> => {
  ...
  for (let i = 0; i < domains.length; i++) {
    ...
    try {
      ...
      if (!(err instanceof APIError) && !(err["name"] === "AssertionError")) {
        // instanceof AssertionError doesn't work :(
        console.error(
          "That Error was not an APIError! All Errors need to be APIErrors. Throwing now to stop the tests...",
        );
      ...
      if (err instanceof APIError && err.original_error instanceof APIError) {
        console.error(
          "Found an APIError as the `original_error` of another APIError! This behaviour is not desirable. Throwing now to stop the tests...",
        );
        throw new Error(
          "Spotted an APIError as the original_error of another APIError!",
        );
      }
    }
    console.log(`\n---- ${i + 1}/${domains.length} ----`);
  ...
  if (!errors.length) {
    ...
    console.log(
      `❌ ${errors.length} test(s) went wrong, here's some information:\n`,
    );

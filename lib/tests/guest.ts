...
import * as Team from "./guest/team.test.js";
...
async function startRunningTests(id: number, secret: string): Promise<void> {
  const api = new API(id, secret, {
    server: "https://osu.ppy.sh",
    retry_on_timeout: true,
    verbose: "all",
  });
  // const api = await API.createAsync(id, secret, undefined, {server: "https://osu.ppy.sh", retry_on_timeout: true, verbose: "all"}) // Async alternative
  await runTests(api, domains);
}

if (process.env.ID === undefined) {
  throw new Error(
    "❌ The ID has not been defined in the environment variables!",
  );
}
if (process.env.SECRET === undefined) {
  throw new Error(
    "❌ The SECRET has not been defined in the environment variables!",
  );
}

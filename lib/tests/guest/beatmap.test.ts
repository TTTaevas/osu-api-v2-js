...
const lookupBeatmap: Test = async (api) => {
  ...
  expect(
    validate(beatmap, "Beatmap.Extended.WithFailtimesOwnersMaxcomboBeatmapset"),
  ).to.be.true;
  return true;
};
...
const getBeatmapDifficultyAttributesOsu: Test = async (api) => {
  const attributes = await api.getBeatmapDifficultyAttributesOsu(125660, [
    "DT",
  ]);
...
const getBeatmapDifficultyAttributesTaiko: Test = async (api) => {
  const attributes = await api.getBeatmapDifficultyAttributesTaiko(388463, [
    "DT",
  ]);
...
const getBeatmapDifficultyAttributesFruits: Test = async (api) => {
  const attributes = await api.getBeatmapDifficultyAttributesFruits(705339, [
    "DT",
  ]);
  expect(attributes.max_combo).to.equal(1029);
  expect(validate(attributes, "Beatmap.DifficultyAttributes.Fruits")).to.be
    .true;
  return true;
};

const getBeatmapDifficultyAttributesMania: Test = async (api) => {
  const attributes = await api.getBeatmapDifficultyAttributesMania(473228, [
    "DT",
  ]);
...
const getBeatmapScores: Test = async (api) => {
  const scores = await api.getBeatmapScores(129891, {
    legacy_only: true,
    mods: ["NM"],
  });
  ...
  scores.forEach(
    (score) =>
      expect(score.mods.find((mod) => mod.acronym === "CL")).to.not.be
        .undefined,
  );
...
const getBeatmapUserScore: Test = async (api) => {
  const score = await api.getBeatmapUserScore(176960, 7276846, {
    mods: ["NM"],
  });
...
const getBeatmapUserScores: Test = async (api) => {
  const scores = await api.getBeatmapUserScores(203993, 7276846, {

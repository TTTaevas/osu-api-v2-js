...
const lookupChangelogBuild: Test = async (api) => {
  ...
  expect(validate(build, "Changelog.Build.WithChangelogentriesVersions")).to.be
    .true;
  return true;
};
...
const getChangelogBuilds: Test = async (api) => {
  const builds = await api.getChangelogBuilds(undefined, undefined, [
    "markdown",
  ]);
  expect(builds).to.have.lengthOf(21);
  builds.forEach((build) =>
    expect(build.created_at).to.be.greaterThan(new Date("2025")),
  );
  builds.forEach((build) =>
    build.changelog_entries.forEach(
      (entry) => expect(entry.message).to.not.be.undefined,
    ),
  );
  builds.forEach((build) =>
    build.changelog_entries.forEach(
      (entry) => expect(entry.message_html).to.be.undefined,
    ),
  );

...
const getComment: Test = async (api) => {
  const comment = await api.getComment(2418884);
  expect(Boolean(comment.users.find((user) => user.id === 32573520))).to.be
    .true;
  expect(validate(comment, "Comment.Bundle")).to.be.true;
  return true;
};

const getComments: Test = async (api) => {
  const comments = await api.getComments();
  expect(validate(comments, "Comment.Bundle")).to.be.true;

  console.log("|", "Beatmapset");
  const commentsBeatmapset = await api.getComments({
    type: "beatmapset",
    id: 1971037,
  });
  ...
  const commentsNewspost = await api.getComments({
    type: "news_post",
    id: 1451,
  });
...
export const tests = [getComment, getComments];

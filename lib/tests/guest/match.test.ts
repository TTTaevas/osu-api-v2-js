...
const getMatch: Test = async (api) => {
  ...
  response_noteams.events.forEach((e) =>
    e.game?.scores.forEach((score) =>
      expect(score.match.team).to.equal("none"),
    ),
  );
  ...
  response_teams.events.forEach((e) =>
    e.game?.scores.forEach((score) =>
      expect(score.match.team).to.not.equal("none"),
    ),
  );
...
export const tests = [getMatch, getMatches];

...
const abort: Test = async (api) => {
  ...
  try {
    ...
    await new API({ ...api, signal: controller.signal }).getUser(2);
  } catch (e) {
    expect(e).to.be.instanceof(APIError);
    if (e instanceof APIError) {
  ...
  throw new Error(
    "Abort test failed, likely that the request was a success (even though it shouldn't have been one)",
  );
};
...
const set_token_on_expires: Test = async (api) => {
  ...
  await new Promise((resolve) =>
    setTimeout(resolve, (seconds_delay + 2) * 1000),
  );
...
export const tests = [timeout, abort, set_token_on_401, set_token_on_expires];

...
const getRoom: Test = async (api) => {
  ...
  expect(validate(room_playlist, "Multiplayer.Room.WithHostRecentparticipants"))
    .to.be.true;
  ...
  expect(validate(room_realtime, "Multiplayer.Room.WithHostRecentparticipants"))
    .to.be.true;
...
const getPlaylistItemScores: Test = async (api) => {
  console.log("|", "Playlist");
  const response_playlist = await api.getPlaylistItemScores({
    id: 5371540,
    room_id: 588230,
  });
  ...
  response_playlist.scores.forEach((score) =>
    expect(score.playlist_item_id).to.equal(5371540),
  );
  response_playlist.scores.forEach((score) =>
    expect(score.room_id).to.equal(588230),
  );
  expect(
    validate(response_playlist.scores, "Multiplayer.Room.PlaylistItem.Score"),
  ).to.be.true;

  console.log("|", "Realtime");
  const response_realtime = await api.getPlaylistItemScores({
    id: 5421279,
    room_id: 591993,
  });
  ...
  response_realtime.scores.forEach((score) =>
    expect(score.playlist_item_id).to.equal(5421279),
  );
  response_realtime.scores.forEach((score) =>
    expect(score.room_id).to.equal(591993),
  );
  expect(
    validate(
      response_realtime.scores,
      "Multiplayer.Room.PlaylistItem.ScoreWithUser",
    ),
  ).to.be.true;
...
const getRoomLeaderboard: Test = async (api) => {
  ...
  expect(validate(response_playlists.leaderboard, "Multiplayer.Room.Leader")).to
    .be.true;
  response_playlists.leaderboard.forEach((leader) =>
    expect(leader.room_id).to.equal(588230),
  );
  if (response_playlists.user_score) {
    expect(
      validate(
        response_playlists.user_score,
        "Multiplayer.Room.Leader.WithPosition",
      ),
    );
  }
  ...
  expect(validate(response_realtime.leaderboard, "Multiplayer.Room.Leader")).to
    .be.true;
  response_realtime.leaderboard.forEach((leader) =>
    expect(leader.room_id).to.equal(591993),
  );
  if (response_realtime.user_score) {
    expect(
      validate(
        response_realtime.user_score,
        "Multiplayer.Room.Leader.WithPosition",
      ),
    );
  }
...
const getRoomEvents: Test = async (api) => {
  ...
  expect(
    validate(
      ...
      "Multiplayer.Room.PlaylistItem.WithDetailsScores",
    ),
  ).to.be.true;

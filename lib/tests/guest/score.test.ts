...
const getScores: Test = async (api) => {
  ...
  const scores_cursor = await api.getScores({
    cursor_string: scores.cursor_string,
  });
  expect(scores_cursor.cursor_string).to.be.a("string");
  if (scores_cursor.scores.length === 0)
    console.log(
      "| ⚠️ Empty array of scores, not throwing but unable to validate",
    );
  ...
  scores_ruleset.scores.forEach((s) =>
    expect(s.ruleset_id).to.equal(Ruleset.fruits),
  );
...
const getScore: Test = async (api) => {
  const score_id = all_scores.length
    ? all_scores[Math.floor(Math.random() * all_scores.length)].id
    : 4892985569;
  if (all_scores.length === 0)
    console.log(
      "| ⚠️ No score gotten from previous test, using an old score instead",
    );
...
export const tests = [getScores, getScore];

...
const getUserScores: Test = async(api) => {
	...
	console.log("| recent")
	const scores_recent = await api.getUserScores(4568537, "recent", undefined, {fails: false, lazer: true}, {limit: 2})
	...
	scores_recent.forEach((score) => expect(score.user_id).to.equal(4568537))
	scores_recent.forEach((score) => expect(score.user.id).to.equal(4568537))

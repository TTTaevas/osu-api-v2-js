...
const getUsers: Test = async (api) => {
  ...
  expect(validate(users, "User.WithCountryCoverGroupsTeamStatisticsrulesets"))
    .to.be.true;
  return true;
};
...
const lookupUsers: Test = async (api) => {
  ...
  expect(users.at(1)?.id).to.equal(7276846);
  users.forEach((user) =>
    expect(user.global_rank?.ruleset_id).to.equal(Ruleset.osu),
  );
  expect(validate(users, "User.WithCountryCoverGroupsTeamGlobalrank")).to.be
    .true;
  return true;
};
...
const getUserScores: Test = async (api) => {
  console.log("| best");
  const scores_best = await api.getUserScores(
    7276846,
    "best",
    undefined,
  ...
  expect(scores_best.at(0)?.pp).to.be.greaterThanOrEqual(
    scores_best.at(1)?.pp!,
  );
  ...
  console.log("| firsts");
  const scores_first = await api.getUserScores(
    6503700,
    "firsts",
    Ruleset.taiko,
    undefined,
  ...
  expect(scores_first.at(0)?.ended_at).to.be.greaterThanOrEqual(
    scores_first.at(1)?.ended_at!,
  );
  ...
  console.log("| recent");
  const scores_recent = await api.getUserScores(
    5795337,
    "recent",
    Ruleset.osu,
    { fails: true, lazer: true },
    { limit: 2 },
  );
  expect(scores_recent).to.have.lengthOf(2);
  expect(scores_recent.at(0)?.ended_at).to.be.greaterThanOrEqual(
    scores_recent.at(1)?.ended_at!,
  );
...
const getUserBeatmaps: Test = async (api) => {
  ...
  expect(beatmapsets.at(-1)?.beatmaps.find((beatmap) => beatmap.id === 1857760))
    .to.not.be.undefined;
...
const getUserPassedBeatmaps: Test = async (api) => {
  const beatmaps = await api.getUserPassedBeatmaps(
    7276846,
    [630897, 316580, 241526],
    {
      converts: true,
      is_legacy: true,
      no_diff_reduction: true,
      ruleset: Ruleset.fruits,
    },
  );
  expect(beatmaps).to.have.lengthOf(5);
  expect(beatmaps.find((beatmap) => beatmap.beatmapset_id === 630897)).to.be
    .undefined;
  expect(beatmaps.find((beatmap) => beatmap.beatmapset_id === 316580)).to.not.be
    .undefined;
  expect(beatmaps.find((beatmap) => beatmap.beatmapset_id === 241526)).to.not.be
    .undefined;
...
const getUserMostPlayed: Test = async (api) => {
  ...
  expect(playcounts.at(0)?.beatmapset.title_unicode).to.equal(
    "furioso melodia",
  );
  playcounts.forEach((pc) => expect(pc.beatmap.id).to.equal(pc.beatmap_id));
  playcounts.forEach((pc) =>
    expect(pc.beatmapset.id).to.equal(pc.beatmap.beatmapset_id),
  );
...
const getUserRanking: Test = async (api) => {
  ...
  for (let i = 0; i < types.length; i++) {
    console.log("|", types[i]);
    const response = await api.getUserRanking(Ruleset.osu, types[i], {
      country: "FR",
    });

...
const testUserStuff = async (): Promise<boolean> => {
	...
	let a3 = await <Promise<ReturnType<typeof api.getUserScores> | false>>attempt("getUserScores (best): ", api.getUserScores(user_id, "best", 5))
	if (!isOk(a3, !a3 || (a3.length === 5 && validate(a3, "Score.WithUserBeatmapBeatmapset")))) okay = false
	let a4 = await <Promise<ReturnType<typeof api.getUserScores> | false>>attempt("getUserScores (firsts): ", api.getUserScores(6503700, "firsts", 3))
	...
	let a5 = await <Promise<ReturnType<typeof api.getUserScores> | false>>attempt(
		"getUserScores (recent): ", api.getUserScores(9269034, "recent", 1, undefined, true))
	...
	let a6 = await <Promise<ReturnType<typeof api.getUserBeatmaps> | false>>attempt("getUserBeatmaps: ", api.getUserBeatmaps(user_id, "guest"))
	if (!isOk(a6, !a6 || (a6.length === 1 && validate(a6, "Beatmapset.Extended.WithBeatmapExtended")))) okay = false
	let a7 = await <Promise<ReturnType<typeof api.getUserMostPlayed> | false>>attempt("getUserMostPlayed: ", api.getUserMostPlayed(user_id))
	if (!isOk(a7, !a7 || (a7[0].beatmapset.title === "furioso melodia" && validate(a7, "Beatmap.Playcount")))) okay = false

	let a8 = await <Promise<ReturnType<typeof api.getUserRecentActivity> | false>>attempt("getUserRecentActivity: ", api.getUserRecentActivity(7562902, 25))
	if (!isOk(a8, !a8 || (a8.length <= 25 && validate(a8, "Event.AnyRecentActivity")))) okay = false
	let a9 = await <Promise<ReturnType<typeof api.getUserKudosu> | false>>attempt("getUserKudosu: ", api.getUserKudosu(user_id, 5))

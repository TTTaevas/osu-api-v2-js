...
const testUserStuff = async (): Promise<boolean> => {
	...
	if (!isOk(a2, !a2 || (a2.length === 2 && validate(a2, "UserWithCountryCoverGroupsStatisticsrulesets", generator)))) okay = false
	...
	if (!isOk(a9, !a9 || (a9.length === 5 && validate(a9, "KudosuHistory", generator)))) okay = false
...
const testBeatmapStuff = async (): Promise<boolean> => {
	...
	if (!isOk(b2, !b2 || (b2.length === 2 && validate(b2, "BeatmapExtended", generator)))) okay = false
	...
	if (!isOk(b11, !b11 || (b11.length >= 100 && validate(b11, "BeatmapPack", generator)))) okay = false
...
const testChangelogStuff = async (): Promise<boolean> => {
	...
	if (!isOk(c2, !c2 || (c2.length === 4 && validate(c2, "ChangelogBuildWithUpdatestreamsChangelogentries", generator)))) okay = false
	let c3 = await <Promise<ReturnType<typeof api.getChangelogStreams> | false>>attempt("getChangelogStreams: ", api.getChangelogStreams())
	if (!isOk(c3, !c3 || (c3.length > 2 && validate(c3, "UpdateStreamWithLatestbuildUsercount", generator)))) okay = false
...
const testMultiplayerStuff = async (): Promise<boolean> => {
	...
	if (!isOk(d6, !d6 || (d6[0].id > 111250329 && validate(d6, "MatchInfo", generator)))) okay = false
...
const testRankingStuff = async (): Promise<boolean> => {
	...
	if (!isOk(e1, !e1 || (e1[0].kudosu.total > 10000 && validate(e1, "UserWithKudosu", generator)))) okay = false
	...
	if (!isOk(e5, !e5 || (e5.length >= 132 && validate(e5, "Spotlight", generator)))) okay = false
...
const testHomeStuff = async (): Promise<boolean> => {
	...
	if (!isOk(f3, !f3 || (f3.title === "Rules" && validate(f3, "WikiPage", generator)))) okay = false
